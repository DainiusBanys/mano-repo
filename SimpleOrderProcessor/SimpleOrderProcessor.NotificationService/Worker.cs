...
namespace SimpleOrderProcessor.NotificationService
{
    public class Worker : BackgroundService
    {
        ...
        public Worker(ILogger<Worker> logger, IConfiguration configuration)
        {
            ...
            _queueName = _configuration["NotificationService:QueueName"];
            _routingKey = _configuration["NotificationService:RoutingKey"];
        }
        ...
        protected override Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            consumer.Received += (model, ea) =>
            {
                ...
                try
                {
                    ...
                    _logger.LogInformation($" [x] NotificationService received OrderId: {order.OrderId}. Sending notification to {order.CustomerEmail}...");
                    // Simulate sending email/SMS
                    Task.Delay(500).Wait();
                    _logger.LogInformation($" [x] NotificationService finished sending notification for OrderId: {order.OrderId}");
                ...
                {
                    _logger.LogError(ex, $"Error sending notification for message: {message}");
